      ...
      {!isTelegramApp && (
        <div className="space-y-3">
          <div className="flex items-center gap-2 px-1">
            <label className="text-[12px] font-black text-white uppercase tracking-[0.2em]">Юзернейм для связи</label>
          </div>
          <input 
            value={username}
          ...
          {(['sbp', 'card'] as const).map((method) => (
            <button 
              ...
              className={`h-24 rounded-3xl border-4 transition-all flex flex-col items-center justify-center relative overflow-hidden ${
                paymentMethod === method ? 'bg-amber-500/20 border-amber-500 shadow-lg' : 'bg-white/5 border-white/10 opacity-70'
              }`}
            >
